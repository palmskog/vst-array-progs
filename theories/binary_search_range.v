...
Definition _binary_search : ident := 63%positive.
Definition _binary_search_range : ident := 61%positive.
Definition _four : ident := 64%positive.
Definition _from : ident := 54%positive.
Definition _high : ident := 58%positive.
Definition _key : ident := 56%positive.
Definition _len : ident := 62%positive.
Definition _low : ident := 57%positive.
Definition _main : ident := 66%positive.
Definition _mid : ident := 59%positive.
Definition _mid_val : ident := 60%positive.
Definition _s : ident := 65%positive.
Definition _to : ident := 55%positive.
Definition _t'1 : ident := 67%positive.

Definition f_binary_search_range := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_a, (tptr tint)) :: (_from, tint) :: (_to, tint) ::
                (_key, tint) :: nil);
  fn_vars := nil;
  ...
  fn_body :=
(Ssequence
  (Sset _low (Etempvar _from tint))
  (Ssequence
    (Sset _high
      (Ebinop Osub (Etempvar _to tint) (Econst_int (Int.repr 1) tint) tint))
    (Ssequence
      (Swhile
        (Ebinop Ole (Etempvar _low tint) (Etempvar _high tint) tint)
        (Ssequence
          (Sset _mid
            (Ebinop Oadd (Etempvar _low tint)
              (Ebinop Odiv
                (Ebinop Osub (Etempvar _high tint) (Etempvar _low tint) tint)
...
(Ssequence
  (Scall (Some _t'1)
    (Evar _binary_search_range (Tfunction
                                 (Tcons (tptr tint)
                                   (Tcons tint
                                     (Tcons tint (Tcons tint Tnil)))) tint
                                 cc_default))
    ((Etempvar _a (tptr tint)) :: (Econst_int (Int.repr 0) tint) ::
     (Etempvar _len tint) :: (Etempvar _key tint) :: nil))
...
((___builtin_ais_annot,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_binary_search_range, Gfun(Internal f_binary_search_range)) ::
 (_binary_search, Gfun(Internal f_binary_search)) :: (_four, Gvar v_four) ::
 (_main, Gfun(Internal f_main)) :: nil).

Definition public_idents : list ident :=
(_main :: _four :: _binary_search :: _binary_search_range ::
